    ...
    async generateContinuation(basePhrase, context, lastMessage, swearProbability, swearPhrases = []) {
        try {
            ...
            const prompt = `Контекст: Ты - полуумный гусь, который отвечает на сообщения в чате.
                           ...
                           3. Выбери 2-3 НАИБОЛЕЕ ПОДХОДЯЩИЕ фразы к текущей теме
                           4. Составь ОДНО короткое предложение, используя выбранные фразы
                           5. Ответ должен продолжать тему разговора
                           6. Если не находишь подходящих фраз - верни "Гусь молчит..."
                           
                           Важно:
                           - Используй только те фразы, которые подходят по смыслу
                           - Лучше использовать меньше фраз, но более уместно
                           - Ответ должен быть НЕ ДЛИННЕЕ 15 слов
                           - Приоритет на осмысленность ответа, а не количество использованных фраз
