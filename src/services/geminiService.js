...
class GeminiService {
    ...
    async generateContinuation(baseText, inputText, allowSwears) {
        try {
            const prompt = `Контекст: Ты - полуумный гусь, который отвечает на сообщения в чате.
                           У тебя есть готовое предложение из фраз: "${baseText}"
                           Учитывая контекст сообщения: "${inputText}"
                           
                           Правила:
                           1. НЕЛЬЗЯ менять смысл исходного предложения
                           2. Можно добавлять только союзы и предлоги для связки
                           3. Можно немного менять окончания для согласования
                           4. Можно добавить эмоциональные частицы (ну, вот, же)
