...
class GeminiService {
    ...
    async generateContinuation(basePhrase, context, lastMessage, swearProbability, swearPhrases = []) {
        try {
            ...
            // Разбиваем контекст на последние сообщения и форматируем их
            const recentMessages = context.split('\n')
                .filter(msg => msg.trim().length > 0)
                .slice(-20)  // Берем последние 20 сообщений
                .map(msg => msg.trim())
                .reverse() // Разворачиваем, чтобы самые старые были вначале
                .map((msg, i) => `[${i + 1}] ${msg}`); // Добавляем нумерацию

            // Анализируем тему разговора
            const conversationContext = recentMessages.length > 0 ? 
                `Тема разговора развивается так:
                 ${recentMessages.join('\n')}
                 
                 Последнее сообщение требует ответа:
                 "${lastMessage}"
                 
                 Учитывай всю историю диалога при ответе. Твой ответ должен:
                 1. Продолжать ТЕКУЩУЮ тему разговора
                 2. Учитывать контекст ВСЕХ предыдущих сообщений
                 3. Быть логически связан с развитием беседы` 
                : 
                `Ответь на сообщение: "${lastMessage}"`;
            ...
            const prompt = `Контекст: Ты - полуумный гусь, который отвечает на сообщения в чате.
                           ...
                           Инструкции:
                           1. ОБЯЗАТЕЛЬНО учитывай ВСЮ историю сообщений при составлении ответа
                           2. Определи главную тему из цепочки сообщений
                           3. Ответ должен быть логическим продолжением ВСЕГО диалога
                           4. Используй ВСЕ предоставленные фразы, объединяя их в ОДНО КОРОТКОЕ предложение
                           5. Твой ответ должен развивать текущую тему разговора
                           6. Если это вопрос - дай краткий ответ с учетом контекста беседы
                           7. Если это утверждение - продолжи мысль, опираясь на предыдущие сообщения
                           ...
                           Важно:
                           - Ответ должен быть логическим продолжением ВСЕЙ беседы
                           - Используй информацию из ВСЕХ предыдущих сообщений
                           ...
                           - Строго следуй развитию темы в диалоге
